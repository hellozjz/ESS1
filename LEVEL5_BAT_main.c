...
#define gain_a2	    0.3409	//ADC_A3	ADCINA3		V3
#define gain_a3	    0.0969	//ADC_A4	ADCINA4		I2
#define gain_a4	    0.097	//ADC_A5	ADCINA5		I1
#define gain_a5	    0.3425	//ADC_A7	ADCINA7		V1
#define gain_b6	    0.3428	//ADC_A6	ADCINA6		V2
...
int		new_mode = 4, new_mode_A = 4, new_mode_B = 4, new_mode_C = 4;
...
if (converter == BAT_CONVERTER)
{
	...
	// Power Dispatch
		if (mode == PRM)
		{
			...
			if (i_set >= 0.5*MAX_INPUT_CUR)	i_set =  0.5*MAX_INPUT_CUR;
			if (i_set <= -0.5*MAX_INPUT_CUR)	i_set = -0.5*MAX_INPUT_CUR;
			iref_A = i_set;
			iref_B = i_set;
			iref_C = i_set;
...
else
{
	...
	// Power Dispatch
		if (mode_A == PRM)
			{
			i_set_A = reference_power_A*1000/3/viA_ave;      // 3 channel
			if (i_set_A >=  0.7*MAX_INPUT_CUR)	i_set_A =  0.7*MAX_INPUT_CUR;
			if (i_set_A <= -0.7*MAX_INPUT_CUR)	i_set_A = -0.7*MAX_INPUT_CUR;
			iref_A = i_set_A;
			}
		ei_A = iref_A - iiA_ave;
	...
	// Power Dispatch
		if (mode_B == PRM)
			{
			i_set_B = reference_power_B*333.3/viB_ave;      //3 channel
			if (i_set_B >=  0.3*MAX_INPUT_CUR)	i_set_B =  0.3*MAX_INPUT_CUR;
			if (i_set_B <= -0.3*MAX_INPUT_CUR)	i_set_B = -0.3*MAX_INPUT_CUR;
			iref_B = i_set_B;
			}
		ei_B = iref_B - iiB_ave;
		...
		if (mode_C == PRM)
			{
			i_set_C = reference_power_C*333.3/viC_ave;      //3 channel
			if (i_set_C >=  0.3*MAX_INPUT_CUR)	i_set_C =  0.3*MAX_INPUT_CUR;
			if (i_set_C <= -0.3*MAX_INPUT_CUR)	i_set_C = -0.3*MAX_INPUT_CUR;
			iref_C = i_set_C;
			}
			ei_C = iref_C - iiC_ave;
